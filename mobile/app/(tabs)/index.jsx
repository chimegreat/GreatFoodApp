...
const HomeScreen = () => {
  ...
  const [featuredRecipe, setFeaturedRecipefeaturedRecipe] = useState(null);
  ...
  const loadData = async () => {
    try {
     ...
     const transformedCategories = apiCategories.map ( (cat, index) => ({ // (cat, index) categories and index
        id : index + 1,
      ...
      setFeaturedRecipefeaturedRecipe(transformedFeatured);

    } catch (error) {
      console.log("Error loaidng the data", error)
    }finally{
      setLoading(false)
    }
  }
